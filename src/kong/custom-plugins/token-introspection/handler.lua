...
function TokenIntrospection:access(config)
  ...
  local jwt_aud = jwt.aud
  -- Check if 'aud' is a table (array)
  if type(jwt_aud) == "table" then
    -- Concatenate array elements into a single string separated by commas
    jwt_aud = table.concat(jwt_aud, ",")
  end
  ...
  utils.set_header("X-Credential-Aud", jwt_aud)
